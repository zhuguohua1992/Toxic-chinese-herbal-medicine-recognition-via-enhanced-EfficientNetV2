...
#             print(targets)
    ...
    eval_results, _ = evaluate(torch.cat(pred_list), torch.cat(target_list), test_cfg.get('metrics'), test_cfg.get('metric_options'))
...
#                 print(targets)
    ...
    eval_results, acc_per_class = evaluate(torch.cat(pred_list),torch.cat(target_list),cfg.get('metrics'),cfg.get('metric_options'), per_cls=True)
#     print(eval_results['confusion'].shape)
#     np.savetxt("confusion_matrix.csv", confusion_matrix, delimiter=",", fmt="%d")
    
    meta['per_cls_acc'].append(acc_per_class)
    
    meta['confusion'].append(eval_results['confusion'])
