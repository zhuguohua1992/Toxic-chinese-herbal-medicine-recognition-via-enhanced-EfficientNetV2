...
class History():
    def __init__(self, dir):
        ...
        self.per_cls_csv_dir = os.path.join(dir,'per_cls_acc.csv')
        self.confusion_csv_dir = os.path.join(dir, 'confusion.csv')
    ...
    def after_epoch(self, meta):
        ...
        val_acc_epoch = []
        train_acc_epoch = []
        epoch_outputs = [['index', 'train_loss', 'train_acc_1', 'train_acc_5', 'train_precision', 'train_recall', 'train_f1-score', 'val_loss', 'val_acc_1', 'val_acc_5', 'val_precision', 'val_recall', 'val_f1-score']]
...
#         print(meta['per_cls_acc']) # self.per_cls_csv_dir
        with open(self.per_cls_csv_dir, 'w', newline='') as f:
            writer = csv.writer(f)
            temp_data_list = []
            for i in range(len(meta['train_info']['train_loss'])):
                temp_data = [i+1] + list(meta['per_cls_acc'][i].numpy())
#                 print(temp_data)
                temp_data_list.append(temp_data)
            writer.writerows(temp_data_list)
        
        
        np.savetxt(self.confusion_csv_dir, meta['confusion'][-1], delimiter=",", fmt="%d")
