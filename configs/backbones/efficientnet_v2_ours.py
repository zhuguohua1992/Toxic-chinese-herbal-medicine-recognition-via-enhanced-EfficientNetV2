...
from .msff import MultiScaleFusion
...
class EfficientNetV2(BaseModule):
    ...
    def __init__(self,
        ...
        self.make_layers()
#         self.msff = MultiScaleFusion(
#             in_channels_list=[
#                 self.arch[2][5],  # layer3
#                 self.arch[4][5],  # layer5
#                 self.arch[6][5],  # layer6 or 7
#             ],
#             out_channels=self.out_channels  # 可调，统一通道维度
#         )
#         self.classifier = nn.Linear(256, num_classes)
#         self.cbams = nn.ModuleDict({
#             '3': CBAM(self.arch[2][5]),
#             '5': CBAM(self.arch[4][5]),
#             '7': CBAM(self.arch[6][5]),
#         })
        self.msff = MultiScaleFusion(
            in_channels_list=[self.arch[2][5], self.arch[4][5], self.arch[6][5]],
            out_channels=self.out_channels
        )
        self.cbam_fused = CBAM(self.out_channels)
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor]:
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i in [3, 5, 7]:
                msff_feats.append(x)
        fused = self.cbam_fused(self.msff(msff_feats))
        return (*outs, fused)
