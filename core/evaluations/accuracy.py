...
def accuracy_torch(pred, target, topk=(1, ), thrs=0., per_cls=False):
    if isinstance(thrs, Number):
        thrs = (thrs, )
        res_single = True
    elif isinstance(thrs, tuple):
    ...
    for k in topk:
        ...
        if res_single:
            res.append(res_thr[0])
            if k == 1:
                top_1_pred = _correct[:k].reshape(-1).float()
        else:
            res.append(res_thr)
#     print(top_1_pred)
#     print(top_1_pred.shape)
    acc_per_class = None
    if per_cls:
        pred_score, pred_label = pred.topk(1, dim=1)
#         print(pred_label.reshape(-1).float())
#         print(pred_label.reshape(-1).float().shape)
#         print(target)
#         print(target.shape)
        top_1_pred = pred_label.reshape(-1).float()
        target = target
        # 计算每个类别的acc
        correct_per_class = torch.zeros(47)
        total_per_class = torch.zeros(47)
        for tar_num in range(len(target)):
            label = target[tar_num]
            pre = top_1_pred[tar_num]
            total_per_class[label] += 1
            if pre == label:
                correct_per_class[label] += 1
        acc_per_class = correct_per_class / total_per_class.clamp(min=1)
#         for i in range(47):
#             print(acc_per_class[i].item())
    return res, acc_per_class


def accuracy(pred, target, topk=1, thrs=0., per_cls=False):
    ...
    res, acc_per_class = accuracy_torch(pred, target, topk, thrs, per_cls)

    return res[0] if return_single else res, acc_per_class
