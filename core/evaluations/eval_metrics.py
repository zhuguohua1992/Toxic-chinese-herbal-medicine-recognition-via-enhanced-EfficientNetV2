...
def evaluate(
    ...
    if 'accuracy' in metrics:
        if thrs is not None:
            acc, acc_per_class = accuracy(results, gt_labels, topk=topk, thrs=thrs, per_cls=per_cls)
        else:
            acc, acc_per_class = accuracy(results, gt_labels, topk=topk, per_cls=per_cls)
    ...
    return eval_results, acc_per_class
