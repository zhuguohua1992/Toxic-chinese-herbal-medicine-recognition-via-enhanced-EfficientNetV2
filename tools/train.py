...
def main():
    ...
    # ---------------- 读取训练 / 验证列表 ---------------- #
    total_annotations = "data_3/train_47.txt"   # 这里如果你有别的路径，再改
    ...
    if args.split_validation:
        ...
        test_annotations = 'data_3/val_47.txt'  # 验证集路径，如有变化请改
        with open(test_annotations, encoding='utf-8') as f:
            val_datas = f.readlines()

    # ---------------- 设备选择：统一用 args.gpu_id ---------------- #
    if args.device is not None:
        device = torch.device(args.device)
    else:
        if torch.cuda.is_available():
            device = torch.device(f'cuda:{args.gpu_id}')  # 例如 cuda:1
        else:
    ...
    # 不再使用 DataParallel，单卡足够
    model.to(device)

    # ---------------- 初始化优化器 & 学习率策略 ---------------- #
    optimizer_type = optimizer_cfg.pop('type')
    optimizer = eval('optim.' + optimizer_type)(params=model.parameters(), **optimizer_cfg)

    lr_type = lr_config.pop('type')
    lr_update_func = eval(lr_type)(**lr_config)

    # ---------------- 构建数据集和 DataLoader ---------------- #
    train_dataset = Mydataset(train_datas, train_pipeline)

    # ⭐ 关键：验证集也使用带 gt_label 的 pipeline，避免 Collect 掉标签
    val_pipeline = copy.deepcopy(train_pipeline)   # 覆盖掉 config 里的 val_pipeline
    ...
    for epoch in range(runner.get('epoch'), runner.get('max_epochs')):
        ...
        train(
            model, runner, lr_update_func, device,
            epoch, data_cfg.get('train').get('epoches'),
            data_cfg.get('test'), meta
        )

        validation(
            model, runner, data_cfg.get('test'), device,
            epoch, data_cfg.get('train').get('epoches'),
            meta
        )
